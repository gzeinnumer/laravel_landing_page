 <!-- achievment -->
 <div class="tittle fw-bold ms-4 mt-5 txt-shadow txt-tittle text-center text-uppercase"><p class="s">achievement</p></div>
 <div class="container">
   <div class="row mt-5 ms-auto me-auto">
     <div class="col text-center bg-achievment rounded-5 p-3 m-3">
       <div></div>
       <h3 class="fw-bold">WOW SI</h3>
       <h3 class="fw-bold">Rangkaian 1</h3>
       <img src="asset/page break.png" width="100%" alt="" />
       <h1 class="fw-nilai">90</h1>
     </div>
     <div class="col text-center bg-achievment rounded-5 m-3 p-3">
       <div></div>
       <h3 class="fw-bold">WOW SI</h3>
       <h3 class="fw-bold">Rangkaian 2</h3>
       <img src="asset/page break.png" width="100%" alt="" />
       <h1 class="fw-nilai">90</h1>
     </div>
     <div class="col text-center bg-achievment rounded-5 m-3 p-3">
       <div></div>
       <h3 class="fw-bold">WOW SI</h3>
       <h3 class="fw-bold">Rangkaian 3</h3>
       <img src="asset/page break.png" width="100%" alt="" />
       <h1 class="fw-nilai">90</h1>
     </div>
     <div class="col text-center bg-achievment rounded-5 m-3 p-3">
       <div></div>
       <h3 class="fw-bold">WOW SI</h3>
       <h3 class="fw-bold">Rangkaian 4</h3>
       <img src="asset/page break.png" width="100%" alt="" />
       <h1 class="fw-nilai">90</h1>
     </div>
   </div>
 </div>
 <!-- end achievment -->